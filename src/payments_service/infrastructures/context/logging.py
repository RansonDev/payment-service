...
Вырезано из faststream/_internal/logger/logging.py, класс ExtendedFilter
(v0.7.5).

Убрано: обрезка message_id по message_id_ln — это подгонка под колоночный
формат вывода FastStream; get_broker_logger и set_logger_fmt с цветным
ColourizedFormatter — цветной вывод противоположен JSON-логам.

Изменено: словарь по умолчанию подмешивается всегда, а не только когда scope
не открыт. В оригинале при открытом scope, где нет какого-то ключа, атрибут
на записи не появлялся вовсе — и строка формата с %(trace_id)s падала
с KeyError прямо в логгере.
