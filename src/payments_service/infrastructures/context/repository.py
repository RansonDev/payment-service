...
Вырезано из faststream/_internal/context/repository.py (v0.7.5).

Убрано:
  * сентинел EMPTY и ContextError из ядра — заменены локальными;
  * __getattr__, отдававший self.get(name) для любого имени. Магия опасная:
    опечатка в имени поля возвращает None вместо AttributeError, и ошибка
    всплывает где-то далеко. Пользуемся явным get();
  * composition.py, context_type.py и resolve.py — это точки интеграции
    с fast_depends, у нас их нет.

Добавлено:
  * bind() — слияние с текущим контекстом вместо замены. В оригинале
    вложенный scope затирает внешний целиком, поэтому trace_id, выставленный
    на HTTP-слое, терялся бы при добавлении payment_id в консьюмере.
