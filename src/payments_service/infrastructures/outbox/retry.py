...
Взято из faststream-outbox (MIT), файл faststream_outbox/retry.py, без изменений
в логике: файл изначально не зависит ни от FastStream, ни от SQLAlchemy, ни от
чего-либо кроме stdlib. Английские комментарии — авторские, сохранены как есть.

Зачем именно этот файл, а не три строки с `base * 2 ** n`:

  * стратегия возвращает либо задержку в секундах, либо None как сигнал
    терминального провала — одна точка решения вместо двух отдельных вопросов
    «сколько ждать» и «пора ли сдаваться»;
  * джиттер применяется до клампа, поэтому max_delay_seconds остаётся
    настоящим потолком;
  * OverflowError от `multiplier ** attempts` при безлимитной конфигурации
    ловится и сатурируется, а не летит наружу;
  * джиттер ограничен двойкой: при большем значении нижняя граница уходит
    в минус и получается мгновенный горячий повтор.

Задержка отсюда НЕ превращается в `datetime.now() + delay` на стороне Python —
смотри OutboxClient.reschedule, где next_attempt_at считает сервер БД.

Для outbox-публикатора нужен ExponentialRetry без max_attempts: событие, которое
не удалось опубликовать, нельзя бросить — брокер лежит, а событие уже
зафиксировано в бизнес-транзакции.
"""
